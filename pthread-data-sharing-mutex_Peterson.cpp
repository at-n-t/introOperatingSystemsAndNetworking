...
void enter_region(int thread_id)
{
   ...
   // Setting thread_id to true to indicate interest
   interested[thread_id] = true;
   
   // TODO: Indicate the thread's turn to execute next
   // Setting turn to thread_id to indicate that its turn is coming up
   turn = thread_id;
   
   // TODO: Busy wait until it is the thread's turn to execute
   // Causing busy waiting until the thead executes
...
void leave_region(int thread_id)
{
   ...
   // Setting thead_id to false to exit
...
void* myFunction(void* arg)
{
	...
	for(unsigned int i = 0; i < 10; ++i) {
      ...
      // Enter the critical region when turn comes
      ...
      //
      //  HINT: You need one function call
      // Leave the critical region when turn ends
