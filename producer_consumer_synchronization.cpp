...
void* takeOrders(void* arg)
{
	...
	for(int i = 0; i < MAX; ++i) {
		...
		// Locking console mutex to enter critical region
		...
		// Unlocking console mutex to exit critical region
		pthread_mutex_unlock(&console_mutex);
	
		// Beginning of critical region 2
		// Locking data mutex to enter critical region
		pthread_mutex_lock(&data_mutex);
		
		// Waits buffer to have space in order to add more to it
		if (num_new_orders == N) {
...
void* processOrders(void* arg)
{
	...
	for(int i = 0; i < MAX; ++i) {
		...
		// Waits if there is no new order
		if(num_new_orders == 0) {
        ...
        // Signals that the buffer still has space
        pthread_cond_signal(&not_full);

		// End of critical region 3
		// Unlocking data mutex to exit critical region
		pthread_mutex_unlock(&data_mutex);
...
int main()
{
	...
	// Waiting the two threads to finish to print out message
